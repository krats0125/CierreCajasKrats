...
{
    public partial class FrmMenudaSuperCaja : Form
    {
        FrmSuperCaja supercaja;
        private Menuda oMenuda;
        public decimal valorentregado = 0;

        public FrmMenudaSuperCaja(FrmSuperCaja supercaja)
        {
            InitializeComponent();
            this.supercaja = supercaja;
            this.oMenuda = new Menuda();
        }
        private void FrmMenudaSuperCaja_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void lbValorTotalEntregado_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (seInserto)
                {
                    ...
                    if(frmSc!= null)
                    {
                        bool actualicionExitosa=new CierreSuperCajaRepository(supercaja).ActualizarCierre(supercaja.idCierre);
